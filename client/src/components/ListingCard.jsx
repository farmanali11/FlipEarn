...
import { ArrowUpRightFromSquare, ArrowUpRightFromSquareIcon, BadgeCheck, LineChart, MapPin, User } from "lucide-react";
    ...
    <div className="relative bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
      ...
      <div className="p-5 pt-8">
        ...
        <div className="flex items-center gap-3 mb-3">
          <span className="text-xs font-medium bg-pink-100 px-3 py-1 rounded-full capitalize">
            {listing.niche}
          </span>
          {listing.country && (
            <div className = "flex items-center text-sm text-gray-500">
            <MapPin className="size-6 mr-1 text-gray-400"/> {listing.country}
            </div>
          )}
          </div>
          { /* Description Section */ }
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={()=>{navigate(`/Listings/${listing.id}`);scrollTo(0,0)}} className="px-7 py-3 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 transition">More Details</button>
